  ...
  return {
    ...
    async config(userConfig, {
      ...
      return {
        ...
        // esbuild: { include: /\.ts$/ },
        resolve: {
          conditions: isVite6OrNewer ? undefined : ['solid', ...(replaceDev ? ['development'] : []), ...(userConfig.mode === 'test' && !options.ssr ? ['browser'] : [])],
          dedupe: nestedDeps,
          alias: [{
            find: /^solid-refresh$/,
    ...
    async transform(source, id, transformOptions) {
      const isSsr = this.environment ? this.environment.config.consumer === 'server' : Boolean(transformOptions?.ssr);
